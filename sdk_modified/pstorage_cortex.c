...
#define BLOCK_COUNT_CHECK(COUNT, SIZE)                                                            \
        if (((COUNT) == 0) ||                                                                     \
            ...
            ((((COUNT) * (SIZE)) > PSTORAGE_FLASH_PAGE_SIZE) && __umod(PSTORAGE_FLASH_PAGE_SIZE, (SIZE))))  \
        {                                                                                         \
            return NRF_ERROR_INVALID_PARAM;                                                       \
        }
    ...
    switch (m_swap_state)
    {
        case STATE_DATA_TO_SWAP_WRITE:
            ...
            retval = sd_flash_write((uint32_t *)(PSTORAGE_SWAP_ADDR),
                                    ...
                                    __udiv(PSTORAGE_FLASH_PAGE_SIZE, sizeof(uint32_t)));
            if (retval == NRF_SUCCESS)
            {
                m_swap_state = STATE_DATA_ERASE;
            }
        ...
        case STATE_NEW_BODY_WRITE:
            ...
            retval = sd_flash_write((uint32_t *)((page_number * PSTORAGE_FLASH_PAGE_SIZE) +
                                    ...
                                    __udiv(p_cmd->size, sizeof(uint32_t)));
            if (retval == NRF_SUCCESS)
            {
                if ((head_word_size == 0) && (tail_word_size == 0))
                {
        ...
        case STATE_SWAP_ERASE:
            ...
            retval = sd_flash_page_erase(__udiv(PSTORAGE_SWAP_ADDR, PSTORAGE_FLASH_PAGE_SIZE));
            if (retval == NRF_SUCCESS)
            {
                m_swap_state = STATE_COMPLETE;
            }
...
static uint32_t cmd_process(void)
{
    ...
    switch (p_cmd->op_code)
    {
        case PSTORAGE_STORE_OP_CODE:
        {
            ...
            if (size < SOC_MAX_WRITE_SIZE)
            {
                retval = sd_flash_write(((uint32_t *)storage_addr),
                                        ...
                                        __udiv(size, sizeof(uint32_t)));
            }
            else
            {
                retval = sd_flash_write(((uint32_t *)storage_addr),
                                        (uint32_t *)p_data_addr,
                                        __udiv(SOC_MAX_WRITE_SIZE, sizeof(uint32_t)));
            }
        }
        break;
        ...
        case PSTORAGE_CLEAR_OP_CODE:
        {
            ...
            {
                page_number = (__udiv(storage_addr, PSTORAGE_FLASH_PAGE_SIZE) + m_round_val);
            ...
            {
                page_number = (__udiv(storage_addr, PSTORAGE_FLASH_PAGE_SIZE));

                uint32_t head_word_size = __udiv((
                    storage_addr -
                    (page_number * PSTORAGE_FLASH_PAGE_SIZE)
                    ), sizeof(uint32_t));

                uint32_t tail_word_size = __udiv((
        ...
        {
            uint32_t page_number = __udiv(storage_addr, PSTORAGE_FLASH_PAGE_SIZE);
                           ...
                           pstorage_handle_t       * p_block_id)
{
    ...
    if (!(__umod(p_module_param->block_size, sizeof(uint32_t)) == 0))
    {
        return NRF_ERROR_INVALID_PARAM;
    }
...
uint32_t pstorage_clear(pstorage_handle_t * p_dest, pstorage_size_t size)
{
    ...
    if (
        !(
            (__umod((p_dest->block_id - m_app_table[p_dest->module_id].base_id),
